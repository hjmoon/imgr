...
def generate_gmm_v1(input_file_path, N):
        f=open(input_file_path,'r')
        lines = f.readlines()[:50]
        f.close()
        image_des = [image_descriptors(image_file) for image_file in lines]
        image_des = [v for v in image_des if v != None]
        print(len(lines),len(image_des))
	words = np.concatenate(image_des) 
...
def get_fisher_vectors_from_folderi_v1(lines, gmm):
	return np.float32([fisher_vector(image_descriptors(file), *gmm) for file in lines])
...
def fisher_features_v1(input_file_path, gmm):
	f=open(input_file_path,'r')
        lines = f.readlines()
        f.close()

	features = {f : get_fisher_vectors_from_folder_v1(lines, gmm) }
	return features
...
def get_args():
    ...
    parser.add_argument('-f' , "--filelist", help="filelist" , default='.')
...
#    gmm = load_gmm(working_folder) if args.loadgmm else generate_gmm(working_folder, args.number)
    gmm = generate_gmm_v1(args.filelist, args.number)
#    fisher_features = fisher_features_v1(working_folder, gmm)
